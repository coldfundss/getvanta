...
local function sameteam(player)
    if not cfg['settings']['team check'] then return false end
...
local function getInputPosition()
    if isMobile and mobiletouch.active then
        return mobiletouch.position
    end
...
local function getcamlocktarget()
    if camlockactive and currenttarget then
        ...
        if player and not playerknocked(player) and not sameteam(player) then
...
local function triggerbot()
    ...
    if currenttarget then
        ...
        if sameteam(player) then return end
...
uis.TouchStarted:Connect(function(touch, processed)
    mobiletouch.position = Vector2.new(touch.Position.X, touch.Position.Y)
...
grm.__index = LPH_NO_VIRTUALIZE(function(self, key)
    if not checkcaller() and self == mouse and cfg['silent aimbot']['enabled'] then
        if key == "Hit" then
            ...
            if sameteam(player) then return oldindex(self, key) end
...
if isMobile then
    createMobileButton("esp", "ESP", 0, function(state)
        cfg['esp']['enabled'] = state
        if state then
    ...
    createMobileButton("speed", "SPD", 1, function(state)
        if cfg['speed modifications']['enabled'] then speedenabled = state end
    end)
    createMobileButton("superjump", "SJ", 2, function(state)
        if cfg['super jump']['enabled'] then superjumpenabled = state end
    end)
    createMobileButton("triggerbot", "TB", 3, function(state)
    ...
    createMobileButton("silentaim", "SA", 5, function(state)
    ...
    do
        ...
        sg2.Name = "VantaMobile_rapidfire"
        sg2.ResetOnSpawn = false
        sg2.IgnoreGuiInset = true
        sg2.DisplayOrder = 999
        sg2.Parent = localplayer.PlayerGui
        local rfBS = BS + 4
